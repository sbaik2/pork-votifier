...
function toSshBigint(value) {
  ...
  var result = Buffer.alloc(length + 4);
  var offset = 0;
...
function createSshPublicKey(rsa) {
  ...
  var header = toSshBigint(Buffer.from("ssh-rsa", UTF8));
  var result = Buffer.alloc(header.length + m.length + e.length);
  var offset = 0;
...
function sshFingerprint(sshKey, sshEncoding, outEncoding) {
    ...
    var result = Buffer.from(hash.digest(BINARY), BINARY);
...
function createSigner(algorithm) {
    ...
    function sign(privateKey, outEncoding) {
        var hashBuf = Buffer.from(hash.digest(BINARY), BINARY);
